...
def display():
    ...
    if 0 <= cowcount and cowcount <= 5:
        for i in cowlocation:
            drawCow(i, True)
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG, cowcount, cowlocation, cow2wld
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            ...
            ################################################3
            isDrag=H_DRAG;
            if 0 <= cowcount and cowcount <= 5 and cursorOnCowBoundingBox:
                # save point
                cowcount += 1
                cowlocation.append(cow2wld.copy())
                if cowcount == 6:
                    isDrag = 0
            elif cowcount == -1 and cursorOnCowBoundingBox:
